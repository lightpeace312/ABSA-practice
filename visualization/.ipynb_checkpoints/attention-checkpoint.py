...
vis_html = """
   <script src="files/lightpeace312/ABSA-practice/visualization/require.js">
           
        
          requirejs.config({
            paths: {
              d3: 'files/lightpeace312/ABSA-practice/visualization/d3.min.js',
              jquery: 'files/lightpeace312/ABSA-practice/visualization/jquery-3.4.0.min.js',
            },
          });
    </script>
...
# re_js = open(os.path.join(__location__, 'require.js')).read()
...
def show(asp_text, ctx_text, atts):
  atts = resize(atts)
  attention = _get_attention(
      asp_text, ctx_text, atts)
  att_json = json.dumps(attention)
  _show_attention(att_json)

def _show_attention(att_json):

  display.display(HTML(vis_html),Javascript('window.attention = %s' % att_json),Javascript(vis_js))
...
def _get_attention(asp_text, ctx_text, atts):
...
#     return [ha.T.tolist() for ha in full_att]
  ...
  def get_ctx_asp_attention():
     print(atts)
     att = np.transpose(atts, (0, 2, 1))   
     print([ha.T.tolist() for ha in att])
     return [ha.T.tolist() for ha in att]
    
  attentions = {
      'all': {
          'att': get_ctx_asp_attention(),
          'top_text': ctx_text,
          'bot_text': asp_text,
      },
      'ctx_asp': {
